...
dateFormat = function (format, timestamp, capitalize) {
	...
	var date = new Date(timestamp - getTZOffset(timestamp)),
		...
		replacements = extend({
			...
			// Day
		    'a': lang.shortWeekdays[day], // Short weekday, like 'Mon'
			'A': lang.weekdays[day], // Long weekday, like 'Monday'
...
defaultOptions = {
	...
	global: {
		useUTC: true,
		//timezoneOffset: 0,
		canvasToolsURL: 'https://www.senka.me/scripts/canvas-tools.min.js',
		VMLRadialGradientURL: 'https://www.senka.me/content/vml-radial-gradient.png'
	},
	chart: {
		//animation: true,
		//alignTicks: false,
	...
	tooltip: {
		...
		headerFormat: '<table style="text-align:right"><tr><td colspan="2" style="font-size: 11px">{point.key}</td></tr>',
...
SVGRenderer.prototype = {
	...
	getStyle: function (style) {
		return (this.style = extend({
		    fontFamily: '"Helvetica Neue", Helvetica, "HGPGothicM", Arial, sans-serif', // default font
			fontSize: '12px'
		}, style));
	},
...
Tooltip.prototype = {
	...
	hide: function (delay) {
		...
		if (!this.isHidden) {
			...
			}, pick(delay, this.options.hideDelay, 300));
			...
			if (area) { // update
				...
				series[areaKey] = series.chart.renderer.path(areaPath)
					.attr({
						fill: pick(
							prop[2],
							Color(prop[1]).setOpacity(pick(options.fillOpacity, 0.2)).get()
						),
						zIndex: 0 // #1069
					}).add(series.group);
			}
